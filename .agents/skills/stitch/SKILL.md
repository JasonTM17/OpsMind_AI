...
Add `STITCH_API_KEY=sk_...` to `~/.Codex/.env` (or `~/.Codex/skills/.env`).
...
```bash
cd ~/.Codex/skills/stitch/scripts && npm install
```

Or run `~/.Codex/skills/install.sh` which handles this automatically.
...
Add to `~/.Codex/.mcp.json` for native design context in Codex:
...
- **No React export** — HTML/Tailwind only; Codex converts to React/Vue components
