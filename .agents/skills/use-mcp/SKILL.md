...
Both paths read MCP servers from `.Codex/.mcp.json`. Both paths preserve main-context budget — the LLM never has to load every tool definition.
...
**Setup.** Symlink `.Codex/.mcp.json` into Gemini's settings location:
...
```bash
cd .Codex/skills/use-mcp/scripts && npm install      # one-time
...
| `scripts/smoke-test.sh` | End-to-end smoke test (no server required) — `bash Codex/skills/use-mcp/scripts/smoke-test.sh` |
...
- **Direct scripts are not optional infrastructure.** They are the only path when Gemini is unavailable, when MCP servers are stdio-only and not registered with Codex's own MCP layer, or when a task needs deterministic tool selection. Built-in `ListMcpResourcesTool` / `ReadMcpResourceTool` only see servers Codex itself has registered — different namespace from `.Codex/.mcp.json`.
