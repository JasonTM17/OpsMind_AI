...
**Task Orchestration (Moderate+ only):** After classifying, create native Codex Tasks for all phases upfront with dependencies. See `references/task-orchestration.md`.
...
5. **Artifact gate:** Write review artifacts from `../_shared/references/workflow-artifacts.md`, then run `node Codex/hooks/workflow-artifact-gate.cjs --stage finalize --artifact-dir <artifact-dir>`.
...
2. **Activate `/ck:project-management` skill (MANDATORY)** → sync plan/task status (if fix is part of a plan), update progress, hydrate Codex Tasks, generate status report
3. `docs-manager` subagent → update `./docs` if changes warrant (NON-OPTIONAL)
4. `TaskUpdate` → mark ALL Codex Tasks `completed` (skip if Task tools unavailable)
