...
1. **Search Strategy**:
   - **Gemini Toggle**: Check `.Codex/.ck.json` (or `~/.Codex/.ck.json`) for `skills.research.useGemini` (default: `false`). If `false` or absent, skip Gemini and use WebSearch directly.
