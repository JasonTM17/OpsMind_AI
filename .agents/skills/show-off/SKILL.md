...
```bash
PREF_SCRIPT=".Codex/skills/show-off/scripts/preferences.js"
[ -f "$PREF_SCRIPT" ] || PREF_SCRIPT="Codex/skills/show-off/scripts/preferences.js"
node "$PREF_SCRIPT" get
```

The helper stores preferences at `~/.Codex/show-off/preferences.json` by default.
...
- If `screenshots=true`, capture each section as images (JPG/PNG) at `assets/showoff/<mission-name>/images/` with ratio-based prefix (`horizontal`, `vertical`, `square`).
  ...
  **IMPORTANT:** Use the parallel capture script for efficiency:
  ```bash
  node .Codex/skills/show-off/scripts/capture-sections.js \
...
The preference helper at `Codex/skills/show-off/scripts/preferences.js` supports:

```bash
# Print resolved user preferences as JSON
node .Codex/skills/show-off/scripts/preferences.js get

# Persist workflow opt-outs
node .Codex/skills/show-off/scripts/preferences.js set --no-screenshots --no-publish --languages en

# Re-enable defaults
node .Codex/skills/show-off/scripts/preferences.js reset
```
...
The parallel capture script at `Codex/skills/show-off/scripts/capture-sections.js` supports:
