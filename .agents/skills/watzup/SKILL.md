...
```bash
node .Codex/skills/watzup/scripts/watzup-scan.cjs --json --fetch
```

When developing from this source repository before install, use `node Codex/skills/watzup/scripts/watzup-scan.cjs --json` only if `.Codex/skills/...` is not present.
