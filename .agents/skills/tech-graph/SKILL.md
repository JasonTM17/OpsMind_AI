...
> Vendored from upstream `fireworks-tech-graph` (yizhiyanhua-ai, MIT). System dep `librsvg` (provides `rsvg-convert`) is installed via `Codex/skills/install.sh`. Do NOT use upstream's `npx skills add` instructions — this skill is part of the CK Engineer stack.

## Vendoring Notes

This skill is vendored — installation is handled by `Codex/skills/install.sh` (no manual `npx skills add` step). The required system dependency `librsvg` (`rsvg-convert` binary) is installed automatically alongside FFmpeg/ImageMagick when `ck install` runs. Drift from upstream is tracked in `~/claudekit/.maintainer/external-sources.json` (Mode A entry) — see AGENTS.md "External Source Drift Watcher" in the monorepo for the update procedure.
...
| 6 | **Codex Official** | Warm cream `#f8f6f3` | Anthropic-style diagrams |
