...
| B-017 | Temporal admission and Phase 9/G4 enablement | V012 source removes the V011 direct-DML bypass and parks exhausted ambiguous post-RPC outcomes as `workflow.reconciliation-required`; static and rollback-only PostgreSQL probes pass, but full fresh/upgrade real-role, atomicity, and latency evidence is absent. No separate reconciliation/alert owner exists for parked or previously claimed ineligible rows. | Platform + database + workflow owners | V012 containment proved on fresh and V001-to-current upgrade paths with real roles and atomic failure injection; predecessor-query `EXPLAIN` and latency threshold evidence; plus a separately authorized read-only exact-workflow reconciliation lane limited to Describe and first-history reads (no Start), with bounded `PENDING` aging/alert behavior | Active |
