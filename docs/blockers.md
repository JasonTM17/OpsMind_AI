...
| B-017 | Temporal admission and Phase 9/G4 enablement | V013 source revokes broad access and grants a separate reconciler role exactly three named functions; exact-workflow Describe plus first-history observation, bounded `PENDING` convergence, aggregate metrics, and seven source-validated alert rules exist. A corrected disposable PostgreSQL real-role run produced 55 PASS markers including cleanup and proved the global exact-three executable set, direct/PUBLIC denial, membership-drift denial, match/two-sample absence/reactivation/mismatch/retry/blocked/retention/exhaustion outcomes, lease fencing/takeover, cross-tenant denial, and four settlement rollback failpoints. A V012-to-V013 upgrade probe also passed exact-three/PUBLIC denial. Admission still lacks revision-bound execution of the wired database gate plus exact-head Maven/Docker/CI and performance/DR gates while `D:` is below 20 GiB, live namespace read-only credential conformance, pinned `promtool` plus live scrape, and an external Alertmanager delivery path. | Platform + database + workflow + SRE owners | Exact merged-head CI/heavy/performance/DR evidence; namespace retention and read-only credential conformance; pinned `promtool` and bounded-label live scrape; configured Alertmanager receiver plus end-to-end page delivery receipt | Active |
...
| B-015 | Dependabot alert #16 for `GHSA-mh99-v99m-4gvg` is dismissed as `tolerable_risk` with a bounded compensating-control rationale. The alert remains intentionally visible in audit policy (`pnpm audit` reports one ignored high finding); the local patch, 5.0.7→5.0.8 override, verifier, owner, and 2026-08-09 review date remain mandatory until the ESLint graph no longer needs `minimatch@3`. | GitHub alert #16 dismissed at 2026-07-27T16:27:49Z with the recorded comment; `node scripts/security/verify-brace-expansion-patch.mjs` reports `BraceExpansionPatch=PASS`, `ExceptionReviewBy=2026-08-09`; PR Quality dependency policy run `30282520687` is green | 2026-07-27 |
...
admission and roadmap G4 until the remaining environment, alert-delivery, and
merged-head heavy evidence exists. Implemented source or a local contract is not
production namespace or paging proof.
