...
1. `IncidentController.timeline` negotiates the Accept matrix from Phase 1: JSON calls existing `timeline`; the vendor calls a new `activityTimeline` method and sets exact `Content-Type`, `Vary: Accept`, and `Cache-Control: no-store`.
...
4. Add concurrent V009 indexes on `(organization_id, project_id, incident_id, occurred_at, event_id)` plus the matching `.sql.conf` `executeInTransaction=false`. On failure, stop rollout, inspect `pg_index.indisvalid`, drop an invalid index concurrently, repair failed Flyway history only after evidence capture, and retry; never modify applied migration bytes.
5. Capture representative high-cardinality `EXPLAIN (ANALYZE, BUFFERS, FORMAT JSON)` evidence. Require branch-bounded index access and no full-ledger materialization; revise SQL/index order if the gate fails.
...
- V008-to-V009 upgrade plus fresh migration run; record valid-index catalog state, query plan, migration duration, append p95 delta, and index bytes.
...
- High: concurrent index build fails or leaves an invalid index. Mitigation: non-transactional script config, migration-before-code order, catalog validation, and forward-recovery runbook.
