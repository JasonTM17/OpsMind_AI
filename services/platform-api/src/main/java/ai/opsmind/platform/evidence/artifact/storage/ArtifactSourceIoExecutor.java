...
final class ArtifactSourceIoExecutor implements AutoCloseable {
    ...
    void detachCleanup(ManagedArtifactSource source) {
        ...
        try {
            ...
            // If the bounded detached queue is saturated, make the source terminal
            // immediately instead of allowing a later replay to leak it.
            source.abort();
        }
    }
    ...
    public void close() {
        reads.shutdownNow();
        for (Runnable abandoned : cleanup.shutdownNow()) {
