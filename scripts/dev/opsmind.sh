...
load_environment() {
    ...
    while IFS= read -r line || [ -n "$line" ]; do
        ...
        case "$key" in
            POSTGRES_PASSWORD|POSTGRES_APP_PASSWORD|POSTGRES_DISPATCHER_PASSWORD|POSTGRES_AI_RUNTIME_PASSWORD|AI_RUNTIME_DATABASE_PASSWORD|SPRING_DATASOURCE_PASSWORD|MINIO_ROOT_PASSWORD|DEEPSEEK_API_KEY)
                if [ -n "$value" ]; then
            ...
            AI_RUNTIME_BODY_RECEIVE_TIMEOUT_SECONDS|AI_RUNTIME_PORT|TOOL_GATEWAY_PORT|\
            POSTGRES_PORT|REDIS_PORT|MINIO_API_PORT|MINIO_CONSOLE_PORT|\
            POSTGRES_DB|POSTGRES_USER|POSTGRES_APP_USER|POSTGRES_DISPATCHER_USER|POSTGRES_AI_RUNTIME_USER|SPRING_PROFILES_ACTIVE|SPRING_DATASOURCE_URL|\
...
assert_application_compose_configuration() {
    ...
    if [ "$POSTGRES_PASSWORD" = "$POSTGRES_APP_PASSWORD" ] ||
       [ "$POSTGRES_PASSWORD" = "$POSTGRES_DISPATCHER_PASSWORD" ] ||
       [ "$POSTGRES_PASSWORD" = "$POSTGRES_AI_RUNTIME_PASSWORD" ] ||
       [ "$POSTGRES_APP_PASSWORD" = "$POSTGRES_DISPATCHER_PASSWORD" ] ||
       [ "$POSTGRES_APP_PASSWORD" = "$POSTGRES_AI_RUNTIME_PASSWORD" ] ||
       [ "$POSTGRES_DISPATCHER_PASSWORD" = "$POSTGRES_AI_RUNTIME_PASSWORD" ]; then
...
case "$command_name" in
    dev|up) ;;
    migrate) unset POSTGRES_PASSWORD POSTGRES_APP_PASSWORD POSTGRES_DISPATCHER_PASSWORD POSTGRES_AI_RUNTIME_PASSWORD AI_RUNTIME_DATABASE_PASSWORD MINIO_ROOT_PASSWORD DEEPSEEK_API_KEY ;;
    *) unset POSTGRES_PASSWORD POSTGRES_APP_PASSWORD POSTGRES_DISPATCHER_PASSWORD POSTGRES_AI_RUNTIME_PASSWORD AI_RUNTIME_DATABASE_PASSWORD SPRING_DATASOURCE_PASSWORD MINIO_ROOT_PASSWORD DEEPSEEK_API_KEY ;;
esac
